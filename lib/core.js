...
function getClassList () {
  ...
  if (num > 0) {
    var c = null;
    var classes = Buffer.alloc(ref.sizeof.pointer * num);
...
function createObject (val, type, name) {
  ...
  // ref.writeObject does not work for us any more.
  // after lenghtly investigation, I gave up.
  // so no more caching of objects.
...
objc.objcStorageKey = Buffer.alloc(1);
